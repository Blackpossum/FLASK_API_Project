...
def test_update_animal(client_with_employees):
    ...
    response = client_with_employees.put('/1', json=update_data)
    assert response.status_code == 200
...
def test_delete_employee(client_with_employees):
    ...
    # add a sample employee to database
        sample_employee = employee(name= 'Bethany',age=27, specialization= 'zoology and environment study', section_operation ='amphibious and mammalia')
        db.session.add(sample_employee)
        db.session.commit()

    # send delete request 
        response = client_with_employees.delete('/1')
